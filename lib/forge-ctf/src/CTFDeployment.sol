...
abstract contract CTFDeployment is Script {
    ...
    function getAdditionalAddress(uint32 index) internal view returns (address) {
        return getAddress(index + 2);
    }

    function getPrivateKey(uint32 index) private view returns (uint) {
    ...
    function getAddress(uint32 index) private view returns (address) {
